...
#' Gated-Linear-Unit keras lambda layer
#'
#' Computes gated temporal convolutions as in: `hl(X) = (X∗W+b) ⊗ σ(X∗V+c)``,
#' where `X` is a data matrix, `W` is a weight kernel, and `b`,`c` are biases.
#' `⊗` is the element-wise multiplication operator
#'
#' @references https://arxiv.org/abs/1612.08083
#' @export
layer_glu <-
  function(object,
           filters,
...
#'
#' Builds upon `layer_glu()`, combining `n` layers into a single block with
